# Squad Team

> outfront-copilot-workshop — OutFront Media Order Management System

## Coordinator

| Name | Role | Notes |
|------|------|-------|
| Squad | Coordinator | Routes work, enforces handoffs and reviewer gates. |

## Members

| Name | Role | Charter | Status |
|------|------|---------|--------|
| Keaton | Lead | .squad/agents/keaton/charter.md | 🏗️ Active |
| Verbal | Frontend Dev | .squad/agents/verbal/charter.md | ⚛️ Active |
| Fenster | Backend Dev | .squad/agents/fenster/charter.md | 🔧 Active |
| Hockney | Tester | .squad/agents/hockney/charter.md | 🧪 Active |
| McManus | DevOps Strategist | .squad/agents/mcmanus/charter.md | ⚙️ Active |
| Kobayashi | Technical Writer | .squad/agents/kobayashi/charter.md | 📝 Active |
| Redfoot | Competitive Analyst | .squad/agents/redfoot/charter.md | 🔍 Active |
| Scribe | Scribe | .squad/agents/scribe/charter.md | 📋 Active |
| Kujan | Context Scout | .squad/agents/kujan/charter.md | 🔎 Active |
| Strausz | Session Reporter | .squad/agents/strausz/charter.md | 📄 Active |
| Edie | Copilot Customization Expert | .squad/agents/edie/charter.md | 🛠️ Active |
| Ralph | Work Monitor | — | 🔄 Monitor |

## Project Context

- **Project:** outfront-copilot-workshop
- **User:** Copilot
- **Stack:** Java 17, Spring Boot 3.2.5, Maven, H2/SQL Server, JUnit 5
- **Domain:** Order Management System for outdoor advertising (billboard/digital signage equipment)
- **Created:** 2026-04-14
